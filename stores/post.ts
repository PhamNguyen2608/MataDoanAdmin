...
export const usePostsStore = defineStore({
  ...
  actions: {
    async getModal(postId: string) {
      await useBaseFetch(`/api/v1/admins/posts/${postId}`, {method: 'GET'})
      .then((res: any) => {
        this.item = res.data.post
      });
    },
    async getModals(params: object) {
      await useBaseFetch('/api/v1/admins/posts', {method: 'GET', params})
      .then((res: any) => {
        this.list = res.data
      });
    },
    async actionPost(postId: string, data: object) {
      ...
      .then((res: any) => {
        ...
        this.list[index].is_active = res.data.post.is_active
      });
    },
    async deletePost(postId: string) {
      await useBaseFetch(`/api/v1/admins/posts/${postId}`, {method: 'DELETE'})
      .then((res: any) => {
        const postId = res.data.post._id;
        const index = this.list.findIndex(post => post._id === postId);
